...
- PDF 본문 자연어 reflow: PyMuPDF `text` 모드는 PDF의 시각적 줄마다 줄바꿈을 넣어 문장이 토막나므로, 추출 시 **블록(문단) 단위로 줄을 재결합**해 자연스럽게 읽히도록 한다. 줄 끝 하이픈으로 끊긴 단어는 다음 조각이 소문자면 하이픈을 제거해 잇고(`represen-tation`→`representation`), 대문자면 하이픈을 유지한다(`self-Attention`). 문단 사이는 빈 줄로 구분해 원문 패널의 `pre-wrap` 렌더가 문단으로 보이게 한다. (DOM 텍스트 == `paper.text`라 하이라이트 문자 오프셋도 이 텍스트 기준으로 일관 유지)
