...
- 노트와 PDF 원본은 FastAPI 백엔드에 저장하고, 서버 미연결 시 localStorage로 폴백합니다.
...
| 원문 보기 | 추출 텍스트 하이라이트, 저장된 PDF 원본 iframe 보기 |
| 하이라이트 | 문자 오프셋 기반 저장, 의미 라벨, 원문/목록 필터 |
| 용어 사전 | 드래그한 용어 추가, 직접 설명 작성, 선택형 AI 설명 |
| 리뷰 작성 | 5문항 요약 템플릿, 질문 기록, 5단계 진행률 |
| 저장 | SQLite 기본, `DATABASE_URL` 설정 시 PostgreSQL, localStorage 오프라인 폴백 |
| 내보내기 | Markdown 다운로드, 브라우저 인쇄 기반 PDF 저장 |
