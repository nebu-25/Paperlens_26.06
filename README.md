...
- 하이라이트는 **원문 내 문자 오프셋(위치)으로 저장**되어, 원문 패널에 **노란 마크로 영구 표시**되고 재열람 시 복원됩니다(FS-02 위치 기반). 원문 패널은 `pre-wrap` 단일 컨테이너로 렌더해 DOM 텍스트와 추출 본문이 1:1 일치합니다. (실제 PDF 캔버스 뷰어 렌더는 후속 단계)
