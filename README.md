...
- 서버 미연결 시 IndexedDB 기반 계정별 로컬 캐시로 임시 보관합니다.
...
| 기능 | 현재 상태 |
| --- | --- |
| 논문 등록 | PDF 업로드, DOI/PDF URL 등록, 기존 노트에 PDF 본문 연결, URL SSRF 방어 |
| 업로드 UX | 업로드/텍스트 추출/메타정보 확인/노트 생성 진행률 표시 |
| 원문 보기 | 추출 텍스트 하이라이트, 2단 컬럼 PDF reflow, 저장된 PDF 원본 캔버스 뷰어 |
| 하이라이트 | 문자 오프셋 기반 저장, 의미 라벨, 원문/목록 필터 |
| 용어 사전 | 드래그한 용어 추가, 직접 설명 작성, 선택형 AI 설명 |
| 리뷰 작성 | 5문항 요약 템플릿, 질문 기록, 5단계 진행률 |
| 그림/표/수식 탐색 | 추출 원문 위치와 PDF 캡션/페이지 이동 버튼 분리, 벡터 표 셀·수식 후보 인덱스, 기존 저장 PDF 구조 인덱스 보강 |
| PDF 영역 메모 | PDF 위 표·그림·수식·임의 영역을 좌표로 지정해 추가 탐색 도구에서 메모·수정·삭제 |
...
- PDF URL 등록은 공용 인터넷의 `http/https` PDF만 허용합니다. `localhost`, 사설망, link-local, metadata IP, private DNS 해석 결과, redirect 우회는 차단하며 로컬 파일은 PDF 업로드로 등록합니다.
- PDF 원문 추출은 2단 한국어 논문에서 컬럼 헤딩과 같은 높이의 짧은 문장 꼬리가 잘못된 위치로 들어가는 경우를 줄이도록 좌표 기반 reflow를 우선 보정합니다.
- 그림/표 네비게이터는 문자 추출 원문 위치와 PDF 뷰어 캡션/페이지 위치를 별도 버튼으로 제공합니다. 표처럼 이미지 객체가 잡히지 않아도 캡션 좌표를 PDF 참조로 사용할 수 있습니다.
- PDF 원본은 표 서식·수식 표기·그림 배치의 시각적 정본입니다. 평문, OCR 비교 후보, 벡터 표 셀, 수식 후보는 원본을 덮어쓰지 않는 보조 인덱스입니다.
- PDF 영역 메모는 인용 목적을 가진 문장 하이라이트와 분리합니다. 표·그림·수식에서 확인한 내용은 `추가 탐색 도구`에 저장하며 인용 후보 보드에 자동으로 들어가지 않습니다.
- 브라우저 임시 저장은 계정별 IndexedDB 캐시를 우선 사용하고 원문 텍스트를 별도 저장합니다. 로그아웃 시 미동기 변경이 있으면 먼저 서버 저장을 시도하고, 성공한 경우에만 해당 계정 캐시를 삭제합니다.
