...
- 설문 URL: https://forms.gle/WrYxvAt6RQqxVia29
- 리뷰 노트 Markdown/PDF, 라이브러리 취합 Markdown, 연구 질문 문서 Markdown 내보내기 후 1회 설문 참여 모달을 표시합니다.
- 로그아웃 완료 후에도 설문 참여 모달을 표시합니다. 로그아웃 자체는 막지 않습니다.
- 로그아웃 설문은 사용자가 숨기거나 설문 참여를 완료하지 않았다면 로그아웃 때마다 다시 표시할 수 있습니다.
- 사용자가 `설문 참여하기`를 누르면 `localStorage`에 완료 상태를 저장해 이후 다시 표시하지 않습니다.
- 사용자가 `이 데모 세션에서는 다시 보지 않기`를 선택하고 닫으면 `sessionStorage`에 저장해 해당 세션에서 다시 표시하지 않습니다.
...
- 엔진: `auto` 모드는 영문 텍스트 힌트가 있으면 RapidOCR을 먼저 시도하고, 그 외에는 NAVER CLOVA OCR API를 먼저 시도한 뒤 RapidOCR이 준비되어 있으면 fallback으로 재시도합니다. fallback이 준비된 `auto` 모드에서는 브라우저/Render 요청 시간 초과를 피하기 위해 CLOVA 1회 대기 시간을 최대 30초로 제한합니다. CLOVA timeout이 반복되면 임시로 `OCR_PROVIDER=rapidocr`로 바꿔 CLOVA를 우회할 수 있습니다. Render 메모리 초과가 발생하면 `OCR_PROVIDER=clova`로 전환해 RapidOCR 모델 로딩을 막는 것이 우선 회피책입니다. `/api/diagnostics`의 `ocr.providers`와 `ocr.configured`로 provider 준비 상태를 확인합니다.
- CLOVA OCR: 서버는 저장된 PDF를 페이지별 PNG로 렌더링해 General 도메인 API Gateway Invoke URL로 전송하고, 응답의 OCR 좌표를 기존 1단/2단 reflow 파이프라인으로 재구성합니다. CLOVA가 단일 언어 중심이라 한국어 문서 우선 provider로 사용합니다. 표 인식 옵션은 별도 과금 때문에 요청하지 않습니다. 프론트는 OCR을 한 번의 긴 요청으로 보내지 않고 `start_page`/`page_count=1` 단위로 순차 호출해 Render 인스턴스 메모리 사용량을 낮춥니다.
- RapidOCR: 영어 fallback까지 운영하려면 OCR 활성화 배포에서 `requirements-ocr.txt`를 추가 설치해야 합니다. Render buildCommand 예: `pip install -r requirements.txt && pip install -r requirements-ocr.txt && pip uninstall -y opencv-python && pip install --force-reinstall --no-deps "opencv-python-headless>=4.9"`. 무료/소형 Render 인스턴스의 메모리 초과를 줄이기 위해 기본 OCR 한도는 10페이지·150dpi이며, 서버는 페이지 렌더링을 약 150만 픽셀 이하 RGB/alpha 없음 pixmap으로 낮춥니다. RapidOCR 엔진은 요청 후 전역 캐시에 남기지 않습니다.
...
문제가 있으면 `OCR_ENABLED=false`로 즉시 되돌릴 수 있습니다(기본 배포·앱 동작에는 영향 없음). `/api/diagnostics`의 `ocr.ready`가 `true`이면 최소 하나 이상의 OCR provider 호출 준비가 끝난 상태입니다.
...
Supabase anon key를 교체한 경우에는 GitHub Actions repository secret `SUPABASE_ANON_KEY`와 GitHub Pages environment variable `VITE_SUPABASE_ANON_KEY`를 함께 갱신한 뒤 프론트를 재배포합니다. `VITE_*` 값은 빌드 산출물에 포함되므로 secret만 바꾸고 재배포하지 않으면 운영 프론트와 smoke 설정이 서로 다른 키를 사용할 수 있습니다.
