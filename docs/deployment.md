...
- 엔진: `auto` 모드는 영문 텍스트 힌트가 있으면 RapidOCR을 먼저 시도하고, 그 외에는 NAVER CLOVA OCR API를 먼저 시도한 뒤 RapidOCR이 준비되어 있으면 fallback으로 재시도합니다. CLOVA timeout이 반복되면 임시로 `OCR_PROVIDER=rapidocr`로 바꿔 CLOVA를 우회할 수 있습니다. `/api/diagnostics`의 `ocr.providers`와 `ocr.configured`로 provider 준비 상태를 확인합니다.
