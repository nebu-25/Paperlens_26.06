...
1. 대표 corpus(`시연확정.pdf` 포함 3~5개)로 PDF 원본, 평문, 표 구조, 수식 후보, OCR 후보를 나란히 점검한다. 제목/초록 순서, 2단 순서, 표 셀, 수식 위치, 문단 경계를 기록한다.
2. 표 원본 보존 2단계: 표 구조가 인식되면 셀 데이터와 함께 PDF 표 영역을 원본 렌더링으로 확인하고, 스캔/감지 실패 표는 사용자가 PDF 영역 메모로 지정할 수 있는지 검증한다.
3. PDF 영역 메모에서 해당 좌표로 다시 이동하는 기능과, 논문 두 편의 PDF 영역 메모를 나란히 비교하는 화면의 범위를 설계한다.
