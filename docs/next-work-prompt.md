...
1. OCR 안전 보완 UX를 실제 논문에서 검수한다. 원문이 있는 PDF는 `□`·대체 문자·정확한 문맥 사이 빈 구간만 바뀌고, 대응이 불명확한 OCR은 적용이 막히는지 확인한다. 스캔 PDF처럼 원문이 0자인 경우에만 `OCR 전체 적용`이 노출되어야 한다.
2. OCR 원문을 공개 Actions 로그에 남기지 않는 정책을 유지한 채, 제목·초록·본문 순서와 한국어 어절을 PDF 원본과 사람 검수한다. 필요하면 검수용 비공개 결과 요약만 추가한다.
3. 모바일 PDF 뷰어의 화면비율 설정 컨트롤이 좁은 화면에서 바를 벗어나는 문제를 재현·수정한다.
4. 수식이 많은 논문이 추가되면 수식 후보의 page/bbox와 PDF 원본 수식 표기를 함께 점검한다.
5. 표 원본 보존 2단계와 PDF 영역 메모 비교 화면은 위 OCR 검수 이후 범위를 설계한다.

최근 완료:

- 2026-08-07 세션에서 PDF corpus 7종을 확인하고, reflow가 놓친 첫 페이지 제목·키워드를 raw 전체 fallback 없이 보존하도록 수정했다. 스캔형 PDF 2종은 실제 운영 OCR smoke에서 기본 추출 0자→OCR 2,444자/1,620자, `good/100`으로 확인했다. 검증 파일은 임시 draft release 자산으로만 전달했고 실행 후 삭제했다.
- OCR 결과 적용 정책을 보완했다. 이제 원문이 비어 있는 스캔 PDF만 전체 OCR을 적용할 수 있고, 원문이 있는 PDF는 앞뒤 문맥이 일치하는 깨진 문자·누락 구간만 안전 patch로 보완한다. 문맥 대응이 없거나 일부 페이지만 처리되면 적용을 막는다. 커밋 `39ef429`; CI `31189287132`, Pages `31189287212`, Production smoke `31189362582` 성공.
- Production smoke는 수동 `check_ocr=true`와 임시 draft release tag를 받아 실제 OCR 비교를 수행할 수 있다. 자동 배포에서는 외부 OCR 비용을 발생시키지 않는다. draft 자산 조회를 위해 workflow의 `contents: write` 권한이 필요하며, workflow 자체는 저장소 쓰기 명령을 실행하지 않는다.
...
- 2026-08-05 세션에서 수식 후보 1단계를 완료했다. PDF 텍스트 레이어의 독립 대입식 행을 `page`·`bbox`·텍스트로 보관하고, 원문 패널 `PDF 수식 후보`에서 PDF 페이지를 열어 실제 수식 표기와 줄바꿈을 확인한다. 수식 후보는 OCR 결과나 LaTex 복원 결과가 아니다. 커밋 `215f2cf`; CI run `30926543966` 배포 성공.
- 2026-08-05 세션에서 pdf.js 뷰어의 영역 메모를 추가했다. 사용자는 표·그림·수식·임의 영역을 드래그해 좌표와 메모를 저장한다. 최종 정책은 인용 목적의 문장 하이라이트와 분리하는 것이다. 영역 메모는 `ReviewNote.pdfAreaNotes`에 저장하고 `추가 탐색 도구`에서 편집·삭제·PDF 페이지 이동으로 관리하며, 인용 후보 보드에는 들어가지 않는다. 이전 `highlights.annotationKind` 데이터는 복원 시 자동 이전한다. 커밋 `dfe59ac`(초기 영역 주석), `0f8eaa0`(하이라이트와 분리); CI run `30929123637` 성공.
...
- 빠른 테스트 문서/저장된 작업 로딩 skeleton은 실제 워크스페이스와 같은 원문/리뷰 2열 비율(`1.7fr / 0.8fr`)을 사용합니다. 단일 중앙 카드로 뜨던 이전 로딩 화면은 기존 작업 화면과 비율이 달라 보여 교체했습니다.
- 빠른 테스트 문서/저장된 작업 로딩 skeleton은 복원 단계(`브라우저 캐시 확인`, `백엔드 준비 확인`, `데모 노트 복원`/`저장된 노트 동기화`, `로컬 복원 대기`)와 경과 시간을 표시합니다. 10초 이상 지연되면 Render 콜드스타트/데모 seed/로컬 캐시 상황을 설명하는 보조 문구를 보여 줍니다.
...
- 2026-07-08에 GitHub Actions `Production smoke` 실패 원인을 운영 Supabase anon key 계열 설정 불일치로 보고 `SUPABASE_ANON_KEY`와 GitHub Pages environment의 `VITE_SUPABASE_ANON_KEY`를 갱신했습니다. 프론트 재배포 후 `Production smoke`가 성공했습니다. 같은 세션에서 T1 일반 리뷰에도 목적 질문 placeholder 안내문구를 추가해 T1~T5 작성 UX를 맞췄고, `cd frontend && npm test -- templates.test.ts`는 11 passed입니다.
