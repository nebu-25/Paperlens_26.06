...
1. PDF 영역 메모 표시·상호작용 개선: 영역 내부를 가리지 않는 색 테두리와 끝 마커를 적용한다. 마커는 색상과 표·그림·수식 아이콘을 함께 보여 주고, 클릭 시 메모를 연다. 메모 삭제 동작은 `하이라이트 해제`가 아니라 `메모 삭제`로 표기한다.
2. 겹치는 PDF 영역 메모는 마커를 겹친 색 점 또는 `+N` 묶음으로 표시한다. 원문 영역은 기본적으로 클릭을 가로채지 않아 PDF를 읽고 선택할 수 있어야 하며, 마커만 메모 열기 동작을 맡는다.
3. 대표 corpus(`시연확정.pdf` 포함 3~5개)로 PDF 원본, 평문, 표 구조, 수식 후보, OCR 후보를 나란히 점검한다. 제목/초록 순서, 2단 순서, 표 셀, 수식 위치, 문단 경계를 기록한다.
4. 표 원본 보존 2단계: 표 구조가 인식되면 셀 데이터와 함께 PDF 표 영역을 원본 렌더링으로 확인하고, 스캔/감지 실패 표는 사용자가 PDF 영역 메모로 지정할 수 있는지 검증한다.
5. PDF 영역 메모에서 해당 좌표로 다시 이동하는 기능과, 논문 두 편의 PDF 영역 메모를 나란히 비교하는 화면의 범위를 설계한다.
6. 운영 브라우저 로그인 smoke와 배포 상태를 확인한다.
