...
OCR은 기본 PDF 추출문을 자동 교체하지 않는다. PDF 추출 품질이 충분하면 해당 원문을 기준으로 유지하고, OCR은 깨진 문자·수식·스캔 영역의 비교 후보로만 표시한다. 후보는 다음 중 하나라도 해당하면 적용을 막는다.

- 전체 페이지를 처리하지 못함
- OCR 텍스트가 기본 원문의 80%보다 짧음
- OCR 추출 품질 점수가 기본 원문보다 낮음
...
- PDF 탭: 수식, 표 서식, 페이지 배치의 기준 원본이다.
- 기본 PDF 추출: 검색, 하이라이트, 요약과 읽기 순서 보정을 위한 평문이다. 수식 객체나 표 서식을 보존한다고 간주하지 않는다.
- 표 구조: 벡터 표에서 감지한 셀 값과 위치를 제공하며, 시각적 재현 모델이 아니다.
- OCR 후보: 깨진 문자·스캔 영역을 확인하는 비교 대상이다. 기본 추출을 자동으로 교체하지 않는다.
...
- 로그인 직후 빠른 노트 불러오기 skeleton이 실제 워크스페이스와 같은 원문/리뷰 2열 비율로 표시된다.
- 로그인 직후 빠른 노트 불러오기 skeleton이 복원 단계와 경과 시간을 표시하고, 10초 이상 지연되면 Render 콜드스타트/데모 seed/로컬 캐시 상황 안내를 표시한다.
...
### 2026-08-07 OCR 안전 보완 적용

- OCR 비교 결과를 적용할 때 기존 원문 전체를 OCR 문자열로 교체하지 않는다.
- 기존 추출문이 비어 있는 스캔 PDF만 OCR 전체 적용을 허용한다.
- 원문이 있으면 `□`·대체 문자처럼 깨진 구간에서 앞뒤 문맥이 OCR 결과와 일치할 때만 해당 구간을 교체한다. OCR 문단이 기존 원문의 두 정확한 문맥 사이 빈 구간에만 있을 때는 삽입 후보로 만든다.
- 문맥 대응이 없거나 처리하지 못한 페이지가 있으면 적용을 막고, 기본 원문 유지 또는 직접 편집을 안내한다.
- 구현: `frontend/src/lib/ocrPatches.ts`가 안전 patch를 만들고, 원문 패널은 전체 교체와 안전 보완을 서로 다른 적용 버튼으로 표시한다.
- 검증: `ocrPatches.test.ts`에 깨진 문자 부분 교체·대응 불가 시 원문 보존·빈 원문 전체 적용 사례를 추가했다. GitHub Actions CI run `31189287132`, Pages run `31189287212`, Production smoke run `31189362582`가 성공했다.
