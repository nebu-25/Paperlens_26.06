...
### 2026-06-27 PDF 추출/내보내기 개선 검증

반영한 범위:

- PDF reflow가 2단 컬럼 페이지에서 같은 y 좌표의 좌우 줄을 섞지 않고, 왼쪽 컬럼 전체를 먼저 읽은 뒤 오른쪽 컬럼으로 넘어가도록 보정했다.
- 줄 reflow 로직을 `_reflow_lines`로 분리해 단일 컬럼과 감지된 컬럼 그룹이 같은 문단 결합 규칙을 사용하게 했다.
- Markdown 내보내기와 브라우저 인쇄용 HTML 내보내기에서 하이라이트를 의미 라벨별로 그룹화했다.
- 인쇄 HTML에는 하이라이트 색상별 class를 붙여 핵심/방법/결과/한계/질문 라벨의 시각 구분을 유지한다.

실행한 로컬 검증:

- `cd backend && ./.venv/bin/pytest tests/test_papers.py`: 60 passed.
- `cd frontend && npm test -- src/lib/export.test.ts`: 9 passed.
- `git diff --check`: 통과.

로컬 개발 확인:

- Vite proxy는 `/api`를 `http://127.0.0.1:8000`으로 전달한다.
- 백엔드가 꺼져 있으면 `[vite] http proxy error`와 `ECONNREFUSED 127.0.0.1:8000`이 발생한다.
- `cd backend && ./.venv/bin/uvicorn app.main:app --host 127.0.0.1 --port 8000 --reload`로 FastAPI를 실행한 뒤 `curl http://127.0.0.1:8000/api/health`가 `{"status":"ok"}`를 반환함을 확인했다.

커밋/푸시:

- `788ba6a Improve paper extraction and export grouping`를 `origin/main`에 푸시했다.
