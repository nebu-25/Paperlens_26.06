...
from app.services.formula_candidates import detect_formula_candidates as _detect_formula_candidates
...
def _extract_pdf_content(
    *,
    ...
    formula_candidates = _detect_formula_candidates(document)
...
def extract_url(
    ...
    safe_url = _validate_public_pdf_url(url)

    try:
        request = urllib.request.Request(safe_url, headers={"User-Agent": settings.crossref_user_agent})
        with _open_public_pdf_url(request, timeout=20) as response:
            content_type = response.headers.get("content-type", "")
    ...
    return _extract_pdf_content(
        content=content,
        filename=_filename_from_pdf_url(safe_url, content_disposition),
        paper_id=paper_id,
        user_id=user_id,
    )
    ...
    if existing is not None:
        ...
        if isinstance(paper, dict) and isinstance(note, dict):
            db.upsert_note(
                user_id,
                paper_id,
                {**paper, "figureImages": figure_images, "tableStructures": table_structures, "formulaCandidates": formula_candidates},
                note,
            )
    return {"figure_images": figure_images, "table_structures": table_structures, "formula_candidates": formula_candidates}
